...
def plot_profile_2var_1x2 (d1a,d1b,d1l,d1c,d1t,d2a,d2b,d2c,d2t,d3a,d3b,d3c,d3t,config):
    ...
    plt.subplots_adjust(top=0.95, bottom=0.16, left=0.09, right=0.91, hspace=0.2, wspace=0.15)
    ...
    for n in d1l:
        axs[0].plot(n[0], n[1], marker='+', markeredgecolor='black', markerfacecolor='None',markersize=4,linestyle='-',color='black',linewidth=0.5,label=n[2])
    for n in d1c:
    ...
    # --- add lines to middle plot [1]

    for n in d2b:
        axs[1].plot(n[0], n[1], color=n[2], linestyle='dashed',linewidth=1.2)
    for n in d2a:
        axs[1].plot(n[0], n[1], color=n[2], linestyle='solid',linewidth=2,label=n[3])
    for n in d2c:
    ...
    # --- add lines to right plot [2]

    for n in d3b:
        axs[2].plot(n[0], n[1], color=n[2], linestyle='dashed',linewidth=1.2)
    for n in d3a:
        axs[2].plot(n[0], n[1], color=n[2], linestyle='solid',linewidth=2,label=n[3])
    for n in d3c:
        axs[2].errorbar(n[0], n[1], zorder=5, xerr=n[2], elinewidth=0.5, fillstyle='none', label=n[3], **marker_style_c)
    for n in d3t:
        axs[2].errorbar(n[0], n[1], zorder=5, xerr=n[2], elinewidth=0.5, fillstyle='none', label=n[3], **marker_style_t)

    # ---- set x-axis title

    axs[0].set_xlabel(config['x_title'][0], fontsize='medium', labelpad=5)
    axs[1].set_xlabel(config['x_title'][1], fontsize='medium', labelpad=5)
    axs[2].set_xlabel(config['x_title'][2], fontsize='medium', labelpad=5)

    # ---- set y-axis title

    axs[0].set_ylabel(config['y_title'][0], fontsize='medium', labelpad=7)
    axs[-1].set_ylabel(config['y_title'][-2], fontsize='medium', labelpad=7)
    ...
    axs[2].set_xlim([config['xmin'][2], config['xmax'][2]])
    axs[2].xaxis.set_major_locator(ticker.MultipleLocator(config['xfrq_major'][2]))
    axs[2].xaxis.set_minor_locator(ticker.MultipleLocator(config['xfrq_major'][2]/config['xfrq_minor'][2]))
    ...
    # ---- y-axis ticks for figures to the right
    ...
    # ---- add legend to middle plot (and, add lidar to legend)

    lidar = Line2D([], [], marker='+',markeredgecolor='black', markerfacecolor='None',markersize=4,linestyle='None')

    handles, labels = axs[1].get_legend_handles_labels()
    handles.append(lidar)
    labels.append('WindCube')
    axs[1].legend(handles=handles, labels=labels,
              loc='best',fontsize='x-small', frameon='True', borderpad=0.5,
...
MICROHH_time_ind = np.argmin(np.abs(MICROHH_time_hr - time2plot))
...
if z_grid == 0 or z_grid ==1:
    ...
    d1a_int = CM1_2dvar['uw_r'] + CM1_2dvar['uw_s']
    d1b_int = CM1_2dvar['uw_s']
    d2a_int = CM1_2dvar['wt_r'] + CM1_2dvar['wt_s']
    d2b_int = CM1_2dvar['wt_s']
    d3a_int = (CM1_2dvar['wq_r'] + CM1_2dvar['wq_s'])*facq 
    d3b_int = CM1_2dvar['wq_s'] * facq
else:
   zzw = zwi
...
d3a_cm1 = rollavg_pandas(d3a_int, num)
d3b_cm1 = rollavg_pandas(d3b_int, num)
...
d3a.append((d3a_cm1[CM1_time_ind], zzw, color_cm1,'CM1'))
d3b.append((d3b_cm1[CM1_time_ind], zzw, color_cm1))
...
if z_grid == 0 or z_grid ==1:
    ...
    d1a_int = DALES_2dvar['uw_r'] + DALES_2dvar['uw_s']
    d1b_int = DALES_2dvar['uw_s']
    d2a_int = DALES_2dvar['wt_r'] + DALES_2dvar['wt_s']
    d2b_int = DALES_2dvar['wt_s']
    d3a_int = (DALES_2dvar['wq_r'] + DALES_2dvar['wq_s'])*facq 
    d3b_int = DALES_2dvar['wq_s'] * facq
else:
   zzw = zwi
...
d3a_dales = rollavg_pandas(d3a_int, num)
d3b_dales = rollavg_pandas(d3b_int, num)
...
d3a.append((d3a_dales[DALES_time_ind], zzw, color_dales,'DALES'))
d3b.append((d3b_dales[DALES_time_ind], zzw, color_dales))
...
if z_grid == 0 or z_grid ==1:
    ...
    d2a_int = FE['wth_t']
    d2b_int = FE['wth_s']
    d3a_int = FE['wqv_t'] * facq
    d3b_int = FE['wqv_s'] * facq
else:
    nt,nk = np.shape(FE['zw_zi'])        # FE only used 186 vertical levels.
    zl = 2790.
    zzw = np.linspace(0,zl,nk) / ztop
    d1a_int = [np.interp(zzw,FE['zw_zi'][i],FE['wth'][i]) for i in range(nt)]
    d1b_int = [np.interp(zzw,FE['zw_zi'][i],FE['wth_s'][i]) for i in range(nt)]
    d2a_int = [np.interp(zzw,FE['zw_zi'][i],FE['wqv'][i]*facq) for i in range(nt)]
...
d1a_fe = rollavg_pandas(d1a_int, num_fe)  # centered time average
d1b_fe = rollavg_pandas(d1b_int, num_fe)
d2a_fe = rollavg_pandas(d2a_int, num_fe)
d2b_fe = rollavg_pandas(d2b_int, num_fe)
d3a_fe = rollavg_pandas(d3a_int, num_fe)
d3b_fe = rollavg_pandas(d3b_int, num_fe)
...
d3a.append((d3a_fe[FE_time_ind], zzw, color_fe,'FastEddy'))
d3b.append((d3b_fe[FE_time_ind], zzw, color_fe))
...
if z_grid == 0 or z_grid ==1:
    if z_grid == 0:
        ...
        zzw = MICROHH_2dvar['zw_zi'][MICROHH_time_ind]
    d1a_int = MICROHH_2dvar['u_w'] + MICROHH_2dvar['u_diff']
    d1b_int = MICROHH_2dvar['u_diff']
    d2a_int = MICROHH_2dvar['thl_w'] + MICROHH_2dvar['thl_diff']
    d2b_int = MICROHH_2dvar['thl_diff']
    d3a_int = (MICROHH_2dvar['qt_w'] + MICROHH_2dvar['qt_diff'])*facq 
    d3b_int = MICROHH_2dvar['qt_diff'] * facq
else:
   zzw = zwi
   nt,nk = np.shape(MICROHH_2dvar['zw_zi'])
   d1a_int = [np.interp(zzw,MICROHH_2dvar['zw_zi'][i],MICROHH_2dvar['thl_w'][i]+MICROHH_2dvar['thl_diff'][i]) for i in range(nt)]
   d1b_int = [np.interp(zzw,MICROHH_2dvar['zw_zi'][i],MICROHH_2dvar['thl_diff'][i]) for i in range(nt)]
   d2a_int = [np.interp(zzw,MICROHH_2dvar['zw_zi'][i],(MICROHH_2dvar['qt_w'][i]+MICROHH_2dvar['qt_diff'][i])*facq) for i in range(nt)]
   d2b_int = [np.interp(zzw,MICROHH_2dvar['zw_zi'][i],(MICROHH_2dvar['qt_diff'][i])*facq) for i in range(nt)]

d1a_microhh = rollavg_pandas(d1a_int, num)  # centered time average
d1b_microhh = rollavg_pandas(d1b_int, num)
d2a_microhh = rollavg_pandas(d2a_int, num)
d2b_microhh = rollavg_pandas(d2b_int, num)
d3a_microhh = rollavg_pandas(d3a_int, num)
d3b_microhh = rollavg_pandas(d3b_int, num)

d1a.append((d1a_microhh[MICROHH_time_ind], zzw, color_microhh,'MicroHH'))  # add data to list
d1b.append((d1b_microhh[MICROHH_time_ind], zzw, color_microhh))
d2a.append((d2a_microhh[MICROHH_time_ind], zzw, color_microhh,'MicroHH'))
d2b.append((d2b_microhh[MICROHH_time_ind], zzw, color_microhh))
d3a.append((d3a_microhh[MICROHH_time_ind], zzw, color_microhh,'MicroHH'))
d3b.append((d3b_microhh[MICROHH_time_ind], zzw, color_microhh))
...
if z_grid == 0 or z_grid ==1:
    ...
    d1a_int = MPAS_2dvar['uw_r'][:,izs:] + MPAS_2dvar['uw_s'][:,izs:]
    d1b_int = MPAS_2dvar['uw_s'][:,izs:]
    d2a_int = MPAS_2dvar['wt_r'][:,izs:] + MPAS_2dvar['wt_s'][:,izs:]
    d2b_int = MPAS_2dvar['wt_s'][:,izs:]
    d3a_int = (MPAS_2dvar['wq_r'][:,izs:] + MPAS_2dvar['wq_s'][:,izs:]) * facq
    d3b_int = (MPAS_2dvar['wq_s'][:,izs:]) * facq
else:
   zzw = zwi
...
d3a_mpas = rollavg_pandas(d3a_int, num) 
d3b_mpas = rollavg_pandas(d3b_int, num)
...
d3a.append((d3a_mpas[MPAS_time_ind], zzw, color_mpas,'MPAS'))
d3b.append((d3b_mpas[MPAS_time_ind], zzw, color_mpas))
...
if z_grid == 0 or z_grid ==1:
    ...
    d1a_int = NCARLES_2dvar['uw_r'] + NCARLES_2dvar['uw_s']
    d1b_int = NCARLES_2dvar['uw_s']
    d2a_int = NCARLES_2dvar['wt_r'] + NCARLES_2dvar['wt_s']
    d2b_int = NCARLES_2dvar['wt_s']
    d3a_int = (NCARLES_2dvar['wq_r'] + NCARLES_2dvar['wq_s']) * facq
    d3b_int = (NCARLES_2dvar['wq_s']) * facq
else:
   zzw = zwi
...
d3a_ncar = rollavg_pandas(d3a_int, num) 
d3b_ncar = rollavg_pandas(d3b_int, num)
...
d3a.append((d3a_ncar[NCARLES_time_ind], zzw, color_ncar,'NCAR-LES'))
d3b.append((d3b_ncar[NCARLES_time_ind], zzw, color_ncar))
...
#   d1a_int = WRFLES_2dvar['uw_r'][:,izs:] + WRFLES_2dvar['uw_s'][:,izs:]
#   d1b_int = WRFLES_2dvar['uw_s'][:,izs:]
    d1a_int = WRFLES_2dvar['u']  # waiting for wrf uw's
    d1b_int = WRFLES_2dvar['u']
    d2a_int = WRFLES_2dvar['wt_r'][:,izs:] + WRFLES_2dvar['wt_s'][:,izs:]
    d2b_int = WRFLES_2dvar['wt_s'][:,izs:]
    d3a_int = WRFLES_2dvar['wq_r'][:,izs:] + WRFLES_2dvar['wq_s'][:,izs:]
    d3b_int = WRFLES_2dvar['wq_s'][:,izs:]
else:
   zzw = zwi
   nt,nk = np.shape(WRFLES_2dvar['zw_zi'])
   ...
   d2a_int = [np.interp(zzw,WRFLES_2dvar['zw_zi'][i], \
                 facq*(WRFLES_2dvar['wq_r'][i]+WRFLES_2dvar['wq_s'][i]))  \
                 for i in range(nt)]                                    
   d2b_int = [np.interp(zzw,WRFLES_2dvar['zw_zi'][i],facq*WRFLES_2dvar['wq_s'][i]) for i in range(nt)]
...
d3a_wrf = rollavg_pandas(d3a_int, num)
d3b_wrf = rollavg_pandas(d3b_int, num)

d1a.append((d1a_wrf[WRFLES_time_ind], zzw, color_wrf,'WRF'))  # add data to list
d1b.append((d1b_wrf[WRFLES_time_ind], zzw, color_wrf))
d2a.append((d2a_wrf[WRFLES_time_ind], zzw, color_wrf,'WRF'))
d2b.append((d2b_wrf[WRFLES_time_ind], zzw, color_wrf))
d3a.append((d3a_wrf[WRFLES_time_ind]*facq, zzw, color_wrf,'WRF'))
d3b.append((d3b_wrf[WRFLES_time_ind]*facq, zzw, color_wrf))

# ---- configure the lidar observations

facz = 1.e-3  # convert [m] -> [km]

d1l.append((lidar_uw[LIDAR_time_ind], lidar_alt*facz, 'WindCube'))

# ---- configure the C130 observations

facz = 1.e-3  # convert [m] -> [km]

d1c.append((c130_uw, c130_alt*facz, c130_uw_std, 'C-130'))
d2c.append((c130_wt, c130_alt*facz, c130_wt_std, 'C-130'))
d3c.append((c130_wq, c130_alt*facz, c130_wq_std, 'C-130'))

#d1c.append((c130_uw_av, c130_alt_uw*facz, c130_uw_lo, c130_uw_hi, 'C-130'))
#d2c.append((c130_wt_av, c130_alt_wt*facz, c130_wt_lo, c130_wt_hi, 'C-130'))
#d3c.append((c130_wq_av, c130_alt_wq*facz, c130_wq_lo, c130_wq_hi, 'C-130'))

# ---- configure the tower observations

facz = 1.e-3  # convert [m] -> [km]

d1t.append((tower_uw, tower_alt*facz, tower_uw_std, 'Tower'))
d2t.append((tower_wt, tower_alt*facz, tower_wt_std, 'Tower'))
d3t.append((tower_wq, tower_alt*facz, tower_wq_std, 'Tower'))

# ---- configure desired ticks/titles

# first declare some variables

xmin = np.empty(3, dtype=object)
xmax = np.empty(3, dtype=object)
xfrq_major = np.empty(3, dtype=object)
xfrq_minor = np.empty(3, dtype=object)
x_title = np.empty(3, dtype=object)

ymin = np.empty(3, dtype=object)
ymax = np.empty(3, dtype=object)
yfrq_major = np.empty(3, dtype=object)
yfrq_minor = np.empty(3, dtype=object)
y_title = np.empty(3, dtype=object)
...
xfrq_major[0] = 0.2
xfrq_minor[0] = 4
x_title[0] = r"$\overline{u'w'}$~~\Large{[m$^{2}$ s$^{-2}$]}"
...
xfrq_major[1] = 0.1
xfrq_minor[1] = 5
x_title[1] = r"$\overline{w'\theta'}$~~\Large{[m K s$^{-1}$]}"

if z_grid == 0:
    y_title[1] = r'$z$~~\Large{[km]}'
else:
    y_title[1] = r'$z / z_i$'
...
x_title[2] = r"$\overline{w'q'}$~~\Large{[m s$^{-1}$\,g kg$^{-1}$]}"
y_title[2] = y_title[0]
...
ax = plot_profile_2var_1x2(d1a, d1b, d1l, d1c, d1t,  # data for plot 0
                           d2a, d2b,      d2c, d2t,  # data for plot 1
                           d3a, d3b,      d3c, d3t,  # data for plot 1
...
ax[2].axvline(x=0.0, color='black', linewidth=0.3)
#ax[2].text(0.95, 0.95, '{time:0>4.0f} LT'.format(time=time2plot*100),
#     transform=ax[2].transAxes,
ax[0].text(0.32, 0.95, '{time:0>4.0f} LT'.format(time=time2plot*100),
      transform=ax[0].transAxes,
      horizontalalignment='right',
      verticalalignment='top',
      fontsize='x-small', color='black')

plt.savefig("../plots/pdf/met_flux_profiles.pdf")
plt.savefig("../plots/png/met_flux_profiles.png", dpi=600, bbox_inches='tight')
